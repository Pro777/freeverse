...
const AUTHOR = {
  ...
  author_slug: "jose-maria-de-heredia",
  century: 19,
  collection_title: "Les Trophées (1893)",
  collection_source_url: "https://fr.wikisource.org/wiki/Les_Troph%C3%A9es",
  text_locale: "fr",
...
async function listPoems() {
  const rendered = await fetchRenderedText("Les Trophées");
  const poems = [];

  for (const match of rendered.matchAll(/href="\/wiki\/(Les_Troph%C3%A9es\/[^"#?]+)"/g)) {
    const encodedTitle = match[1];
    const pageTitle = decodeURIComponent(encodedTitle).replace(/_/g, " ");
    const tail = pageTitle.split("/").pop();
    if (!tail) continue;

    poems.push({
      page_title: pageTitle,
      title: stripTags(tail),
      slug: slugify(tail),
...
function extractTitle(rendered, fallbackTitle) {
  ...
  if (!wsTitle) return fallbackTitle;
  return stripTags(wsTitle[1]).replace(/\s+/g, " ").trim() || fallbackTitle;
}
...
function extractPoem(rendered, pageTitle) {
  ...
  const wsDataIndex = rendered.indexOf('<div id="ws-data"');
  const body = wsDataIndex !== -1 ? rendered.slice(wsDataIndex) : rendered;
    ...
    .map((match) =>
      stripTags(
        match[1]
          ...
          .replace(/<span[^>]*class="lettrine-lettre"[^>]*>/g, "")
          .replace(/<br\s*\/?>/g, "WIKISOURCE_LINEBREAK_TOKEN")
          .replace(/\n+/g, ""),
      )
        .replace(/WIKISOURCE_LINEBREAK_TOKEN/g, "\n")
  ...
  if (!paragraphs.length) throw new Error(`No poem content found for ${pageTitle}`);
  ...
  return yaml.dump(
    {
      ...
      public_domain_rationale:
        "Public domain in the United States: this edition was published in 1893 (pre-1929) and the author died in 1905; text via French Wikisource.",
...
async function main() {
  ...
  if (poems.length !== 123) {
    throw new Error(`Expected 123 poems, found ${poems.length}`);
  }
