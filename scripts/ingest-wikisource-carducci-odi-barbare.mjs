...
const AUTHOR = {
  ...
  author_slug: "giosue-carducci",
  century: 19,
  collection_title: "Odi barbare (1877)",
  collection_source_url: "https://it.wikisource.org/wiki/Odi_barbare",
  text_locale: "it",
...
function extractRedirectTarget(rendered) {
  const redirectMatch = rendered.match(/<link rel="mw:PageProp\/redirect" href="\/wiki\/([^"]+)"/);
  return redirectMatch ? decodeURIComponent(redirectMatch[1]).replace(/_/g, " ") : null;
}

async function fetchRenderedText(pageTitle, seen = new Set()) {
  const url =
    "https://it.wikisource.org/w/api.php?action=parse&format=json&formatversion=2&prop=text&page=" +
    encodeURIComponent(pageTitle);
  const stdout = await fetchText(url);
  const json = JSON.parse(stdout);
  if (!json.parse?.text) throw new Error(`Wikisource parse failed for ${pageTitle}`);
  const redirectTarget = extractRedirectTarget(json.parse.text);
  if (redirectTarget && !seen.has(redirectTarget)) {
    seen.add(pageTitle);
    return fetchRenderedText(redirectTarget, seen);
  }
  return json.parse.text;
}

function keepPage(pageTitle) {
  if (/\/Versioni$/.test(pageTitle)) return false;
  if (/\/Versioni\//.test(pageTitle)) return false;
  if (/\/Note$/.test(pageTitle) || /\/Nota$/.test(pageTitle)) return false;
  if (/Ragioni metriche$/.test(pageTitle)) return false;
  if (/Delle Odi Barbare Libro [IVX]+$/.test(pageTitle)) return false;
  return true;
}

async function listPoems() {
  const rendered = await fetchRenderedText("Odi barbare");
  const poems = [];

  for (const match of rendered.matchAll(/href="\/wiki\/(Odi_barbare\/[^"#?]+)"/g)) {
    const encodedTitle = match[1];
    const pageTitle = decodeURIComponent(encodedTitle).replace(/_/g, " ");
    if (!keepPage(pageTitle)) continue;
    ...
    poems.push({
      ...
      slug: slugify(title),
      source_url: `https://it.wikisource.org/wiki/${encodedTitle}`,
    });
  }

  const deduped = Array.from(new Map(poems.map((poem) => [poem.page_title, poem])).values());
  deduped.sort((a, b) => a.page_title.localeCompare(b.page_title, "it"));
  return deduped;
}
...
function extractPoem(rendered, pageTitle) {
  const poemMatches = [...rendered.matchAll(/<div class="poem">([\s\S]*?)<\/div>/g)];
  if (!poemMatches.length) throw new Error(`No poem block found for ${pageTitle}`);
    ...
    .map((match) =>
      stripTags(
        match[1]
          ...
          .replace(/<span class="numeroriga"[\s\S]*?<\/span>/g, "")
          .replace(/<link rel="mw-deduplicated-inline-style"[^>]*>/g, "")
  ...
  return yaml.dump(
    {
      ...
      public_domain_rationale:
        "Public domain in the United States: this edition was published in 1877 (pre-1929) and the author died in 1907; text via Italian Wikisource.",
...
async function main() {
  ...
  if (poems.length !== 51) {
    throw new Error(`Expected 51 poems, found ${poems.length}`);
  }
